...
//------------------------Parameter----------------------
`define TV_IN_din_a "../tv/cdatafile/c.simd_array.autotvin_din_a.dat"
`define TV_IN_din_b "../tv/cdatafile/c.simd_array.autotvin_din_b.dat"
`define TV_IN_out_r "../tv/cdatafile/c.simd_array.autotvin_out_r.dat"
`define TV_IN_size "../tv/cdatafile/c.simd_array.autotvin_size.dat"
`define TV_IN_opcode "../tv/cdatafile/c.simd_array.autotvin_opcode.dat"
...
parameter opcode_DEPTH = 1;
reg [31 : 0] opcode_OPERATE_DEPTH = 0;
parameter opcode_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter simd_array_continue_addr = 0;
parameter simd_array_auto_start_addr = 0;
...
parameter opcode_data_in_addr = 60;
...
reg [DATA_WIDTH - 1 : 0] mem_opcode [opcode_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_opcode [ (opcode_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * opcode_DEPTH -1 : 0] = '{default : 'hz};
reg opcode_write_data_finish;
...
//write opcode reg
reg [31 : 0] write_opcode_count = 0;
reg write_opcode_run_flag = 0;
reg write_one_opcode_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & din_a_write_data_finish & din_b_write_data_finish & out_r_write_data_finish & size_write_data_finish & opcode_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        opcode_write_data_finish <= 0;
        write_opcode_run_flag <= 0; 
        write_opcode_count = 0;
        count_operate_depth_by_bitwidth_and_depth (opcode_c_bitwidth, opcode_DEPTH, opcode_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            opcode_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_opcode_run_flag <= 1; 
            write_opcode_count = 0;
        end
        if (write_one_opcode_data_done === 1) begin
            write_opcode_count = write_opcode_count + 1;
            if (write_opcode_count == opcode_OPERATE_DEPTH) begin
                write_opcode_run_flag <= 0; 
                opcode_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_opcode
    integer write_opcode_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] opcode_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = opcode_c_bitwidth;
    process_num = 5;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_5_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_opcode_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write opcode data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (opcode_c_bitwidth < 32) begin
                        opcode_data_tmp_reg = mem_opcode[write_opcode_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < opcode_c_bitwidth) begin
                                opcode_data_tmp_reg[j] = mem_opcode[write_opcode_count][i*32 + j];
                            end
                            else begin
                                opcode_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_opcode[write_opcode_count * four_byte_num  + i]!==opcode_data_tmp_reg) begin
                    write (opcode_data_in_addr + write_opcode_count * four_byte_num * 4 + i * 4, opcode_data_tmp_reg, write_opcode_resp);
                    image_mem_opcode[write_opcode_count * four_byte_num + i]=opcode_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_opcode_data_done <= 1;
                @(posedge clk);
                write_one_opcode_data_done <= 0;
            end   
            process_5_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_opcode_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [opcode_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
  integer transaction_idx; 
  integer i; 
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (opcode_c_bitwidth , factor);
  fp = $fopen(`TV_IN_opcode ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_opcode); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < opcode_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_opcode [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
